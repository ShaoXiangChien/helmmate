...
steer Codex, Claude Code, and other local coding agents through real repos,
...
- You use Codex, Claude Code, Cursor, Aider, or local scripts inside real repos.
...
| **Not an agent framework.** | Bring your own agents and prompts. HelmMate coordinates when and how they touch your repos. |
...
for Codex and Claude Code, route work by repo or ticket role, and expose role
personas, model defaults, and engine warnings in the UI.
...
| **A coding agent CLI** | Install and authenticate at least one launch engine, such as Codex or Claude Code. |
...
The easiest setup path is to install the HelmMate skill pack once, then let a
coding agent inspect your repo and write the routine config.
...
Then open your coding agent from the target project folder and paste the setup
prompt generated by the HelmMate onboarding screen.
...
- A more formal engine interface for Codex, Claude, custom scripts, and future
  local agent tools.
