...
// OpenCode `--format json` emits JSON events where completed steps carry
// accounting at `part.tokens`: { total, input, output, cache: { write, read } }.
// Its cost field is a dollar estimate for the step. For the learned token model
// we count input + output + cache writes (same idea as Claude cache-creation);
// OpenCode's `total` includes cache reads, so don't use it directly.
function tokensFromOpenCodePart(part) {
  if (!part || typeof part !== "object" || !part.tokens) return { ...EMPTY_TOKENS };
  const tokens = part.tokens;
  const cache = tokens.cache && typeof tokens.cache === "object" ? tokens.cache : {};
  const input = num(tokens.input);
  const output = num(tokens.output);
  const cacheCreate = num(cache.write);
...
function parseOpenCodeLogTokens(text) {
  let summed = { ...EMPTY_TOKENS };
  let costUSD = null;
  let sawAny = false;
  let limitHit = false;
  ...
  for (const rawLine of text.split("\n")) {
    ...
    if (event.type === "step_finish" && event.part && event.part.tokens) {
      sawAny = true;
      summed = addTokens(summed, tokensFromOpenCodePart(event.part));
      const stepCost = num(event.part.cost);
      if (stepCost > 0) costUSD = (costUSD || 0) + stepCost;
    }
    if (!limitHit) {
      const errText = limitTextFromEvent(event);
      if (errText) {
  ...
  if (sawAny) return { tokens: summed, costUSD, source: "opencode", limitHit, limitMarker };
  return { tokens: { ...EMPTY_TOKENS }, costUSD: null, source: "none", limitHit, limitMarker };
}
...
export function parseLogTokens(logPath, engine = "claude", { startOffset = 0 } = {}) {
  ...
  // Codex/OpenCode logs are different JSONL dialects — parse them separately.
  if (engine === "codex") return parseCodexLogTokens(text);
  if (engine === "opencode") return parseOpenCodeLogTokens(text);
...
export function createRun({
  ...
  const run = {
    ...
    role,
    model,
    // Which CLI worked the ticket ("claude" | "codex" | "opencode"). Drives
    // which log parser applyTokenAccounting uses. Older runs lack it → treated
    // as claude.
    engine,
    pr_url: null,
  };
