...
import { resolveRole, codexEffort, codexModel, opencodeModel, opencodeVariant } from "./roles.js";
import {
  resolveEngine,
  buildClaudeArgs,
  buildCodexArgs,
...
function dispatchFixSession(ticket, kind, prUrl, repo) {
  ...
  // persona file (--model only); Codex/OpenCode fix sessions get the role
  // persona prepended and engine-specific model defaults by role.
  let command;
  let args;
  let runModel;
  if (engine === "codex") {
    ...
    args = buildOpenCodeArgs({
      ...
      model: omodel,
      variant: opencodeVariant(ticket),
      cwd: WORKSPACE_DIR,
    });
    runModel = omodel;
  } else {
