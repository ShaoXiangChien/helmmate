...
import { resolveEngine, buildClaudeArgs, buildCodexArgs } from "./engine.js";
...
function commandPreview({ engine, role, ticket, model, effort, useAgent }) {
  if (engine === "codex") {
...
export function buildLaunchPreview(ticket, options = {}) {
  ...
  const useAgent = engine !== "codex" && agentFileExists(role);
  const effort = engine === "codex" ? codexEffort(ticket) : null;
  const model = engine === "codex" ? codexModel(ticket) : effectiveModel(ticket);
  ...
  return {
    ...
    role: {
      ...
      mode: engine === "codex" ? "prompt preamble" : useAgent ? "claude agent" : "missing persona fallback",
    },
    model: {
      name: model,
      source: engine === "codex" ? source(ticket, "codex_model", "role default") : source(ticket, "model", "persona/default"),
    },
    effort: engine === "codex" ? { name: effort, source: source(ticket, "codex_effort", "role default") } : null,
    command: commandPreview({ engine, role, ticket, model, effort, useAgent }),
    cwd: WORKSPACE_DIR,
    branch,
    worktree,
    promptFile: {
