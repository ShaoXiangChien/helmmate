...
// --- OpenCode engine knobs -------------------------------------------------
//
// OpenCode Go is the open-model lane. Models are pinned explicitly in the
// `opencode-go/<model-id>` form so a ticket never silently falls back to the
// user's global OpenCode config. Per-ticket opencode_model / opencode_variant
// can still opt a run up/down.
...
export const OPENCODE_VARIANTS = ["minimal", "low", "medium", "high", "max"];

export const OPENCODE_MODEL_BY_ROLE = {
...
export const OPENCODE_VARIANT_BY_ROLE = {
  "ios-engineer": null,
...
export function opencodeModel(ticket) {
  if (
    ticket &&
    typeof ticket.opencode_model === "string" &&
    OPENCODE_MODELS.includes(ticket.opencode_model)
  ) {
    return ticket.opencode_model;
  }
  const role = resolveRole(ticket);
  return OPENCODE_MODEL_BY_ROLE[role] || "opencode-go/minimax-m2.7";
}

export function opencodeVariant(ticket) {
  if (
    ticket &&
    typeof ticket.opencode_variant === "string" &&
    OPENCODE_VARIANTS.includes(ticket.opencode_variant)
  ) {
    return ticket.opencode_variant;
  }
  const role = resolveRole(ticket);
  return OPENCODE_VARIANT_BY_ROLE[role] || null;
}
