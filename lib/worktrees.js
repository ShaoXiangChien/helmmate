...
// at dev-board/worktrees/<ticket>/<repo>. Repos with `worktree: false` run in
...
export function ensureWorktree({ repo, ticketId, branch, baseBranch, reuse = false } = {}) {
  ...
  // Make sure the parent dir exists (dev-board/worktrees/<ticket-id>).
  try {
...
// listWorktrees: every dev-board-managed worktree currently on disk, grouped by
// ticket. Reads the filesystem layout (dev-board/worktrees/<ticket-id>/<repo>) so it
