...
export function launchTicket(ticket, options = {}) {
  ...
  // Resolve the engine (claude|codex) + role, then build the subprocess through
  // the engine.js seam (one place wires both CLIs). Codex gets the persona body
  // prepended to the prompt (it has no --agent); Claude passes --agent <role> and
  // lets the persona frontmatter pin the model.
  let command;
  let cliArgs;
  let launchDesc;
  let launchedModel;
