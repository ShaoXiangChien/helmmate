...
export function validateTicketShape(ticket) {
  ...
  if (ticket.opencode_model != null && ticket.opencode_model !== "" && !OPENCODE_MODELS.includes(ticket.opencode_model)) {
    issues.push(issue("error", "bad_opencode_model", `unknown opencode_model: ${ticket.opencode_model}`));
  }
  if (
    ticket.opencode_variant != null &&
    ticket.opencode_variant !== "" &&
    !OPENCODE_VARIANTS.includes(ticket.opencode_variant)
  ) {
    issues.push(issue("error", "bad_opencode_variant", `unknown opencode_variant: ${ticket.opencode_variant}`));
  }
...
export function launchPreflight(ticket, options = {}) {
  ...
  if (engine === "codex") {
    ...
    if (!commandExists(OPENCODE_BIN)) {
      issues.push(issue("error", "missing_opencode", `OpenCode CLI is not available at ${OPENCODE_BIN}`));
    }
  ...
  if (!fs.existsSync(SCRIPTS_PROMPT)) {
    issues.push(issue("warning", "missing_work_prompt", `work prompt not found: ${WORK_PROMPT_REF}`));
  }

  const role = resolveRole(ticket);
  if (!agentFileExists(role)) {
    const fallback =
      engine === "codex" || engine === "opencode"
        ? `${engine === "opencode" ? "OpenCode" : "Codex"} will run without a persona preamble`
        : "Claude will fall back to a plain model run";
    issues.push(issue("warning", "missing_persona", `persona file not found for ${role}: ${agentFilePath(role)}; ${fallback}`));
  }
