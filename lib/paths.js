...
function findConfigPath() {
  if (process.env.DEVBOARD_CONFIG) return path.resolve(process.env.DEVBOARD_CONFIG);
  const candidates = [
    path.resolve(process.cwd(), "devboard.config.json"),
    path.join(BOARD_DIR, "devboard.config.json"),
    path.resolve(BOARD_DIR, "..", "devboard.config.json"),
  ];
...
if (process.env.DEVBOARD_WORKSPACE_DIR) merged.workspaceDir = process.env.DEVBOARD_WORKSPACE_DIR;
if (process.env.DEVBOARD_TICKETS_DIR) merged.ticketsDir = process.env.DEVBOARD_TICKETS_DIR;
if (process.env.DEVBOARD_TICKET_ID_PREFIX) merged.ticketIdPrefix = process.env.DEVBOARD_TICKET_ID_PREFIX;
if (process.env.DEVBOARD_AGENT_DIR) merged.agentDir = process.env.DEVBOARD_AGENT_DIR;
if (process.env.DEVBOARD_MEMORY_QUEUE_DIR) merged.memoryQueueDir = process.env.DEVBOARD_MEMORY_QUEUE_DIR;
if (process.env.DEVBOARD_WORK_PROMPT) merged.workPrompt = process.env.DEVBOARD_WORK_PROMPT;
if (process.env.DEVBOARD_FIX_CI_PROMPT) merged.fixCiPrompt = process.env.DEVBOARD_FIX_CI_PROMPT;
if (process.env.DEVBOARD_FIX_CONFLICT_PROMPT) merged.fixConflictPrompt = process.env.DEVBOARD_FIX_CONFLICT_PROMPT;
if (process.env.DEVBOARD_HOST) merged.host = process.env.DEVBOARD_HOST;
if (process.env.DEVBOARD_PORT) merged.defaultPort = Number(process.env.DEVBOARD_PORT);
if (process.env.DEVBOARD_WIP_LIMIT) merged.defaultWipLimit = Number(process.env.DEVBOARD_WIP_LIMIT);
if (process.env.DEVBOARD_AGENT_MODEL) merged.agentModel = process.env.DEVBOARD_AGENT_MODEL;
if (process.env.DEVBOARD_AGENT_ENGINE) merged.engines.default = process.env.DEVBOARD_AGENT_ENGINE;
if (process.env.DEVBOARD_ENGINES) {
  merged.engines.allowed = process.env.DEVBOARD_ENGINES.split(",").map((item) => item.trim()).filter(Boolean);
}
...
export const ENGINES_CONFIG = {
  ...
  allowed: uniqueStrings(merged.engines.allowed, DEFAULT_CONFIG.engines.allowed),
};
...
export const CODEX_BIN =
  process.env.DEVBOARD_CODEX_BIN || (fs.existsSync(CODEX_APP_BIN) ? CODEX_APP_BIN : "codex");
